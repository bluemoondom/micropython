...
circle(xc,yc,120,colour(243,157,71)) # Large blue circle
...
for p in range(0,360,30):
    hxn, hyn = end_point(p, r)
    LCD.line(120,120,120+hxn,120+hyn,colour(200,200,200))
...
strWeekDays = ('Lu','Ma','Me','Je','Ve','Sa','Di')
...
while True:
    ...
    prnt_st("7",70,186,2,243,157,71)
    prnt_st("5",158,186,2,243,157,71)
    prnt_st("8",42,160,2,243,157,71)
    prnt_st("4",190,160,2,243,157,71)
    prnt_st("9",29,115,2,243,157,71)
    prnt_st("3",200,115,2,243,157,71)
    prnt_st("10",40,70,2,243,157,71)
    prnt_st("2",190,70,2,243,157,71)
    prnt_st("11",70,35,2,243,157,71)
    prnt_st("1",157,35,2,243,157,71)
    prnt_st("12",110,25,2,243,157,71)
    prnt_st("6",115,200,2,243,157,71)

    #vss = Vbat.read_u16()*3.3/65535*2
    #vs = "{:.1f}".format(vss)
    #cntr_st(vs,214,2,155,155,155)
    ...
    circle(xc,yc,9,colour(255,115,0))
    ...
    circle(xc,yc,8,colour(1,102,169))
    ...
    cntr_st("Dominika",78,1,200,200,200) # Message
    ...
    ys = "0"+str(year-2000)
    dd = ds[-2:] +"/"+ms[-2:]+"/"+ys[-2:]
    cntr_st(dd,130,2,155,155,155)

    # Draw Day of Week
    #ws = "{:02}".format(strWeekDays[wd])
    ws = strWeekDays[wd]
    prnt_st(ws,156,100,3,29,118,50)
    #cntr_st(ws,90,3,255,255,0)

    # Draw Digital Time
    hs = "0"+str(h)
    ms = "0"+str(m)
    ts = hs[-2:] +":"+ms[-2:]
    cntr_st(ts,160,2,155,155,155)
